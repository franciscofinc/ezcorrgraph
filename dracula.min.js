    ...
    var edge = {/*
        ...
        draw : function() {
            ...
            edge.fg && edge.fg[move]({path:path}) 
                || (edge.fg = selfRef.path(path).attr({stroke: style && style.stroke || "#000", fill: "none", "stroke-dasharray": style && style.dasharray || "", "stroke-width": style && style.width || 1}).toBack());
            ...
            style && style.label 
                && (edge.label && edge.label.attr({x:(x1+x4)/2, y:(y1+y4)/2}) 
                    || (edge.label = selfRef.text((x1+x4)/2, (y1+y4)/2, style.label).attr({fill: "#000", "font-size": style["font-size"] || "12px", class:"rotulo"})));

            if (style.label !== undefined){
                var box = edge.label.getBBox();
                style && style.label
                    && (edge.caixa && edge.caixa.attr({x: box.x, y: box.y})
                        || (edge.caixa = selfRef.rect(box.x, box.y, box.width, box.height).attr({fill: 'white', "stroke-width": 0})));
                edge.label.toFront();
                //console.log(edge.caixa)
            }
 ...
 *  Dracula Graph 
 ...
 and Drawing Framework 0.0.3alpha
 ...
 */
Graph.Renderer.Raphael = function(element, graph, width, height, estilo) {
    this.width = width || 400;
    this.height = height || 400;
    var selfRef = this;
    this.r = Raphael(element, this.width, this.height);
    this.radius = estilo.tamanho_nó*2 || 40; /* max dimension of a node */
    this.graph = graph;
    this.mouse_in = false;
    ...
    d.onmouseup = function () {
        selfRef.isDrag && selfRef.isDrag.set[0].animate({"fill-opacity": 0.6}, 500);
        selfRef.isDrag && selfRef.isDrag.set[1].animate({"fill-opacity": 1}, 500);
        //console.log(selfRef.isDrag.set.attr())
        //console.log(selfRef.isDrag && selfRef.isDrag.set[1].attr({"fill-opacity": 1}))
        //selfRef[1].attr({"fill-opacity": 1})
...
//            this.r.safari();
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                var ellipse = r.ellipse(0, 0, estilo.tamanho_nó, estilo.tamanho_nó).attr({fill: color, stroke: color, "stroke-width": 2});
                /* set DOM node ID */
                ellipse.node.id = node.label || node.id;
                shape = r.set().
                    push(ellipse).
                    push(r.text(0, 0, node.label || node.id).attr({"font-size": estilo.tamanho_fonte_nó || "16px", "font-weight": "bold", fill:"black", "fill-opacity":1}));
                return shape;
            }
        }
